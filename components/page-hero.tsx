...
export function PageHero({ wordmark, intro = [], introAsTagline = false, artifactId, panel, headingLevel: Heading = 'h1' }: {
  ...
  return <section className="wide editorial-page-hero">
    <div className="editorial-page-heading"><Heading className="font-serif">{wordmark}</Heading><div className={introAsTagline ? 'page-intro is-tagline' : 'page-intro'}>{intro.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div></div>
