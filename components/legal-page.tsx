...
export function LegalPage({ title, current, introduction, sections }: LegalPageProps) {
  return (
    <div className={styles.page}>
      <article className={styles.document} aria-labelledby="legal-title">
        <header className={styles.header}>
          <nav className={styles.navigation} aria-label="Legal pages">
            <Link href="/privacy" aria-current={current === 'privacy' ? 'page' : undefined}>privacy</Link>
            <Link href="/terms" aria-current={current === 'terms' ? 'page' : undefined}>terms</Link>
          </nav>
        ...
        <div className={styles.sections}>
          ...
          <section aria-labelledby="legal-contact">
            ...
            <address className={styles.address}>
              Burgama<br />
              701 Tillery St #12, Mailbox #289<br />
              Austin, TX 78702
            </address>
          </section>
        </div>
